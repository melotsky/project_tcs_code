...
?>
    ...
    <!-- BEAM START -->
    <section id="beam_area" class="group">
        <div id="beam_wrapper" class="group site-main">
            <?php the_field('beam_text_bo', 'option') ?>
        </div>
    </section>
    <!-- BEAM END -->
    <section id="secHHolder" class="group"><header id="masterHeader" class="group site-main">
<?php
}
...
?>
    ...
    <?php
    if ( is_post_type_archive( 'ajde_events' ) or is_singular( 'ajde_events' ) ) : ?>
    <section id="event_banner" class="events__banner">
        <div class="group events__helper">
            <div class="site-main group" id="events__bannerholder">
            <div class="group events__bh">
            
                <?php $image1 = wp_get_attachment_image_src(get_field('banner_1', 'option'), 'banner_event'); ?>
                <?php if(get_field('link_banner_1', 'option') ) { 
                    $link1 = get_field('link_banner_1', 'option');
                    echo "<a href=\"{$link1}\" rel='nofollow'>"; 
                } ?>
                <img src="<?php echo $image1[0]?>" class="eb1" />
                <?php if(get_field('link_banner_1', 'option') ) { 
                    echo "</a>"; 
                } ?>

                <?php $image2 = wp_get_attachment_image_src(get_field('banner_2', 'option'), 'banner_event'); ?>
                <?php if(get_field('link_banner_2', 'option') ) { 
                    $link2 = get_field('link_banner_2', 'option');
                    echo "<a href=\"{$link2}\" rel='nofollow'>"; 
                } ?>
                <img src="<?php echo $image2[0]?>" class="eb2" />
                <?php if(get_field('link_banner_2', 'option') ) { 
                    echo "</a>"; 
                } ?>
            </div>
            <?php if ( is_post_type_archive( 'ajde_events' ) ) : ?>
...
function theHeaderLogo(){
    ...
    $source_id = "Header Contact / Popup Form";    
    if( get_field('enable_popup_form_header', 'option') ) : ?>
        <div id="popupformbtn_mob" class="">
            <div class="popup_form__btn_mob">
                <a href="#" onclick="Intercom('showNewMessage'); return false" class=""><?php the_field('button_label_pff', 'option') ?></a>
            </div>
        </div>
    <?php endif;    
}
...
function navWrapperEnd(){
    ...
    echo "</div>";
    if( get_field('enable_popup_form_header', 'option') ) : ?>
        <div id="popupformbtn" class="popup_form__btn">
            <a href="#" onclick="Intercom('showNewMessage'); return false" class=""><?php the_field('button_label_pff', 'option') ?></a>
        </div>
    <?php endif;
    echo "</div>";
...
function humbergerMennu(){
    echo "<div id=\"hamMenuHolder\" class=\"group sb-toggle-right show-right showRespMenu\">";
...
add_action('my_header', 'humbergerMennu', 4);
